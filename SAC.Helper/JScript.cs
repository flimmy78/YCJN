...
namespace SAC.JScript
{
    ...
    {
        #region 原代码
        public static void ConfirmAndRedirect(string message, string firstUrl, string secondUrl)
        {
            string js = "<script type='text/javascript'>if(confirm('" + message + "')){window.location = '" + firstUrl + "';}else{window.location = '" + secondUrl + "';}</script>";
            ((Page)System.Web.HttpContext.Current.Handler).RegisterClientScriptBlock("错误", js);
        }

        public static void ConfirmAndRedirect(string message, bool isFirst, string toUrl)
        {
            string js;
            if (isFirst)
            {
                js = "<script type='text/javascript'>if(confirm('" + message + "')){window.location = '" + toUrl + "';}</script>";
            }
            else
            {
                js = "<script type='text/javascript'>if(!confirm('" + message + "')){window.location = '" + toUrl + "';}</script>";
        ...
        {
            string js = @"<Script language='JavaScript'>alert('" + msg + "');window.opener=null;window.close();</Script>";
            HttpContext.Current.Response.Write(js);
            HttpContext.Current.Response.End();
        }
...
window.opener.location.href=window.opener.location.href;
        ...
        /// </summary>
        /// <param name="url"></param>
        public static void OpenWebForm(string url, int width, int height, int top, int left)
        {
            ...
            string js = @"<Script language='JavaScript'>
		//window.open('" + url + @"');
		window.open('" + url + "','','height=" + height + ",width=" + width + ",top=" + top + ",left=" + left + ",location=no,menubar=no,resizable=no,scrollbars=yes,status=no,titlebar=no,toolbar=no,directories=no');</Script>";
            /*结束*/
            /*…………………………………………………………………………………………*/
        ...
        public static void Open86(string url)
        {
            /*…………………………………………………………………………………………*/
            /*修改人员:		sxs						*/
            /*修改时间:	2003-4-9	*/
            ...
            string js = @"<Script language='JavaScript'>
		//window.open('" + url + @"');
		window.open('" + url + @"','','height=600,width=800,top=20,left=50,location=no,menubar=no,resizable=no,scrollbars=no,status=yes,titlebar=no,toolbar=no,directories=no');
		</Script>";
            /*结束*/
            /*…………………………………………………………………………………………*/
        ...
        public static void OpenWebForm(string url, int isFullScreen)
        {
            string js = @"<Script language='JavaScript'>";
            if (isFullScreen == 1)
            {
        ...
        {
            string js = @"<Script language='JavaScript'>
window.parent.location.href='" + parentWindowUrl + "'; window.parent.location.reload;</Script>";
            HttpContext.Current.Response.Write(js);
        }
        /// <summary>		
        /// 函数名:GotoOpenerWindow	
        /// 功能描述:返回父窗口	
        /// 处理流程:
        /// 算法描述:
        /// 作 者: 孙洪彪
        ...
        /// </summary>
        /// <param name="parentWindowUrl">父窗口</param>		
        public static void GotoOpenerWindow(string parentWindowUrl)
        {
            string js = @"<Script language='JavaScript'>
window.opener.location.href='" + parentWindowUrl + "'; window.opener.location.reload;</Script>";
            HttpContext.Current.Response.Write(js);
        }
        /// <summary>		
        ...
        #region Js新方法
        /// <summary>
        /// Js刷新当前页，不能用cs重定向，因为报错：无法在发送 HTTP 标头之后进行重定向
        /// </summary>
        public static void ReloadCurrentPage()
        {
            if (HttpContext.Current != null)
            {
                //js重定向，不能用cs重定向，因为报错：无法在发送 HTTP 标头之后进行重定向 
        ...
        public static void ExecuteJs(string jsText)
        {
            if (HttpContext.Current != null)
            {
                HttpContext.Current.Response.Write(string.Format(@"<script>{0}</script>", jsText));
            }
        }
        ...
        /// Js跳转到一个页面 得到Js代码
        /// </summary>
        public static string RedirectToUrlJsText(string url)
        {
            return string.Format(@"<script type='text/javascript'>window.location='{0}'</script>", url);
        }

        ///// <summary>
        ///// 在 UpdatePanel 中弹出JavaScript小窗口
        ///// </summary>
        ///// <param name="js">窗口信息</param>
        //public static void AlertForUpdatePanal(string message, System.Web.UI.UpdatePanel updatePanel)
        //{
        ...
        //    {
        //        ScriptManager.RegisterStartupScript(updatePanel, page.GetType(), "alert", script, true);
        //    }
        //}
